...
describe('Filters', () => {
  it('type filter functioning', () => {
    ...
    expect(store.getters['query/filters/type/rendered'].value?.toLowerCase()).toBe(Types.any);
    store.commit('query/setRouteParams', { type: Types.text });
    expect(store.getters['query/filters/type/rendered'].value?.toLowerCase()).toBe(Types.text);
