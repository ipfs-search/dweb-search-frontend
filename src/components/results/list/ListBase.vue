<template>
  <v-container>
    ...
    <v-row dense v-if="$parent.resultsTotal !== 0">
      <slot />
    </v-row>
    <v-row dense justify="center" v-if="this.$parent.loading">
      <v-progress-circular indeterminate />
    </v-row>
    <!--     PAGINATION -->
    <!-- TODO: pagination panel falls behind social media bar without margin-bottom -->
    <div class="my-16" style="margin-bottom: 135px !important" v-if="!anyFileType && !infinite">
      <v-pagination v-model="$parent.queryPage" :length="pageCount" total-visible="9" />
    </div>
  </v-container>
</template>
